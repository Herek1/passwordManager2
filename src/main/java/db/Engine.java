...
import db.utils.EngineSettings;
import db.utils.TableMapper;
...
public class Engine {
    ...
    public Engine(){
        ...
        this.DBURL = "jdbc:postgresql://localhost:5432/";
        this.statement = new Statement();
        if (!doesDatabaseExist("passwords", "postgres", "admin", DBURL)) {
            createDatabase("passwords", "postgres", "admin", DBURL);
        }
        engineConnection = db.connectToDb("passwords", "postgres", "admin");
        engineSettings.setDbName("passwords");
    }
